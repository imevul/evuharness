...
const DATABASE_URL = process.env.EVUHARNESS_TEST_DATABASE_URL;

const describeStores = DATABASE_URL === undefined || DATABASE_URL === '' ? describe.skip : describe;

let pool: Pool;
let sessions: PostgresSessionStore;
let grants: PostgresGrantStore;

describeStores('postgres stores', () => {
  beforeAll(async () => {
    pool = await openPool({ connectionString: DATABASE_URL! });
  });
  ...
  beforeEach(async () => {
    await pool.query(
      'TRUNCATE sessions, grants, settings, user_profile, memories RESTART IDENTITY',
    );
  ...
  describe('session persistence', () => {
    ...
    it('round-trips a session provider preference and agent pin', async () => {
      const original = {
        ...record(),
      ...
      await sessions.upsert(original);
      const loaded = await sessions.get(SESSION);
      expect(loaded?.provider).toEqual(original.provider);
      expect(loaded?.agentId).toBe('guide');
      expect(loaded?.compaction).toEqual(original.compaction);
    });
  ...
  describe('grant semantics parity', () => {
    async function addGrant(
      decision: Parameters<typeof grantForDecision>[0],
      workspaceId?: string,
    ) {
    ...
    it('isolates a session grant from another session', async () => {
      ...
      await expect(
        resolveGrant(grants, { sessionId: 'other', tool: TOOL, digest }),
      ).resolves.toEqual({ allowed: false });
    });
  ...
  describe('settings persistence', () => {
    it('round-trips a provider including the secret, and survives a reopen', async () => {
      ...
      const again = new PostgresSettingsStore({ pool });
